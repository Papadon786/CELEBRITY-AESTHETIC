...
/**
 * Ensures primary standard accounts exist in PostgreSQL database:
...
export async function getStaffActivityHistory(params?: { staffId?: string; limit?: number }): Promise<StaffActivityItem[]> {
  ...
  for (const p of payments) {
    ...
    activityStream.push({
      ...
      staffEmail: p.receivedBy?.email || "staff@celebrityaesthetic.com",
  ...
  for (const it of inventoryTx) {
    ...
    activityStream.push({
      ...
      staffEmail: it.performedBy?.email || "staff@celebrityaesthetic.com",
  ...
  for (const apt of appointments) {
    ...
    const docName = apt.doctor?.name || "the attending doctor"
    ...
    activityStream.push({
      ...
      staffEmail: apt.createdBy?.email || "staff@celebrityaesthetic.com",
