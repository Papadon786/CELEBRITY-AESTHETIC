...
const TREATMENT_CATEGORIES: TreatmentCategory[] = ["skin", "hair", "aesthetics", "pmu"];
            ...
            if (link.href === "/treatments") {
              return (
                <li key={link.href} className="group relative">
                  ...
                  {/* Hover (and keyboard-focus) dropdown of the 4 treatment
                      categories — opacity/translate transition rather than
                      unmounting, so it's reachable by keyboard via
                      group-focus-within without layout jumping. */}
                  <div
                    className="invisible absolute left-1/2 top-full z-10 w-56 -translate-x-1/2 translate-y-1 pt-3 opacity-0 transition-all duration-200 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:visible group-focus-within:translate-y-0 group-focus-within:opacity-100"
                  >
                    <ul className="overflow-hidden rounded-md border border-gold/30 bg-ivory shadow-[0_12px_32px_rgba(23,23,23,0.12)]">
                      {TREATMENT_CATEGORIES.map((category) => (
                        <li key={category}>
                          <Link
                            href={`/treatments?category=${category}`}
                            className="block px-5 py-3 font-grotesk text-[13px] uppercase tracking-widest2 text-charcoal/80 transition-colors hover:bg-sage hover:text-gold-dark"
                          >
